...
- symbol: 证券代码/交易代码。**必须仔细查找**原文中与"代码""股票代码""交易编号""code""symbol""证券代码"等列对应的简短字母数字（如美股 AAPL/RAM/QQQ，港股 00700，A股 600519）。**注意**：券商表格中"代码"列通常在名称列旁边，是一个简短的纯字母或数字组合。如果确实没有任何代码，才设为 null。**绝对禁止忽略表格中的代码列**。
...
- 券商交易确认格式：序号 交易日期 交易编号 股票名称 股票代码 买入数量 成交价 交易金额。**symbol 取"股票代码"列的值**。`,
          },
          {
            role: 'user',
            content: text,
