...
### 八、上线后复验发现：数据源在不同网络环境下行为不同（重要）

推送后打到**线上接口**复验，发现港股历史**仍返回 0 条**，而同一份代码在本机是 22 条 —— 说明"本地跑通"并不等于"线上可用"。

**定位手法**：拿**同一标的不同代码路径**对比。A股 `period` 路径走新浪 → 30 条 ✓；A股**区间**路径走东财 K 线 → 0 条 ✗。由此锁定：`push2his.eastmoney.com`（东财 K 线）**从 Cloudflare Workers 取不到数据**（返回空 `data`），Yahoo `GC=F` 同样取不到；而 `api.fund.eastmoney.com` 与 Yahoo 美股接口在两边都正常。

**顺带澄清一个长期问题**：线上"黄金有 21 条"以前看着正常，其实那是 **KV 缓存里的旧值**；缓存版本递增强制回源后，才暴露出"黄金历史在生产环境一直拿不到数据"。即本次改动是**暴露**而非造成该问题。

**修法（多源兜底 + 竞速）**：

- 港股 / A股：东财 K 线 + **腾讯日K**（`web.ifzq.gtimg.cn/appstock/app/fqkline/get`，`qfq` 前复权）。**实测两源逐日完全一致**（港股 07709 各 22 条、偏差 0.00%；A股 600519 末值 1253.800 = 东财 1253.8），可互换、不会"换个源曲线就变样"。
- 黄金：Yahoo GC=F + **新浪外盘 GC 日K**（JSONP）。同一标的、日收盘偏差 ≤2%；黄金历史只取**形状**（末点等比缩放到 AU9999 现价），偏差可接受，总好过整块黄金从曲线消失。
- 新增 `raceNonEmpty()`：并发发起多个源、**谁先返回非空就用谁**；全部失败才返回空，但**必须 `console.error` 打日志** —— 原实现是 `catch → []` 完全静默，这正是本次排查最耗时的原因（线上只能看到"曲线少一块"，查不出所以然）。
- 缓存版本递增到 **v3**（KV `HISTORY_CACHE_VERSION` + 前端 `SEG_CACHE_VERSION`）。

**验证**：本地 22/22 通过；且测试时东财恰好正在限流本机 IP（连续探测后直接断连），因此这次验证走的正是"**主源失败 → 兜底源顶上**"的路径，真实覆盖了生产场景。
